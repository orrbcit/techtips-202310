...
function writeHikes() {
    ...
    hikesRef.add({
        ...
        last_updated: firebase.firestore.Timestamp.fromDate(new Date("November 11, 2023"))
    });
}
    ...
    db.collection(collection).get()   //the collection called "hikes"
        .then(snap => {
            ...
            snap.forEach(doc => { //iterate thru each doc
                ...
                var docID = doc.id;
                var timestamp = doc.data().last_updated.toDate();
                ...
                newcard.querySelector('.card-text').innerHTML = details + " " + timestamp;
...
// this function is called automatically each time the page loads to clean up firestore of old hikes
var THRESHOLD = 30;
function removeOldHikes() {
    db.collection("hikes")
        .get()
        .then(function (list) {
            list.forEach(function (doc) {
                ...
                if (daysDifference > THRESHOLD) {
                    db.collection("hikes").doc(doc.id).delete()
                        .then(() => {
                            console.log(`Deleted hike: ${doc.id}`);
                            //refresh the page after deletion
                            location.reload();
                        })
                        .catch((error) => console.error(`Error deleting hike ${doc.id}:`, error));
                }
            });
        })
